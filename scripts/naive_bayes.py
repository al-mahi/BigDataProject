...
if __name__ == '__main__':
    ...
    rdd = sc.textFile("/training_data/training.1600000.processed.noemoticon-small.csv")
    ...
    rawLabelTweetDataFrame = spark.createDataFrame(text2, ["labels", "tweets"])
    ...
    filteredDataFrame = remover.transform(tokenized).select("labels", "filtered")

    ngram = NGram(n=1, inputCol="filtered", outputCol="ngrams")
    ngramDataFrame = ngram.transform(filteredDataFrame)
    ngramDataFrame.select("labels", "ngrams").show(truncate=False)
    data = ngramDataFrame.select("labels", "ngrams")
